          ...
          >
            ...
            <div className="p-6 space-y-6 flex-1 overflow-y-auto custom-scrollbar">
              ...
              <div className="flex gap-2">
                ...
                <button
                  onClick={() => startCreate()}
                  className="px-4 py-2 bg-emerald-600 text-white font-black uppercase text-xs tracking-widest hover:bg-emerald-700 transition-all border border-emerald-500/20 shadow-xl shadow-emerald-500/10 flex items-center gap-2 shrink-0 rounded-xl"
                >
                  ...
                  <div className="grid gap-3 font-mono">
                    ...
                    {isCreating && !editParentId && (
                      <div className="p-4 border-2 border-primary bg-primary/5 flex items-center gap-3 animate-in fade-in slide-in-from-top-2">
                        ...
                        />
                        <button onClick={() => handleSave()} className="p-1.5 bg-emerald-600 text-white hover:bg-emerald-700 rounded-lg" title={t("admin.banners.categories.save_btn")}><Save size={16} /></button>
                        <button onClick={() => setIsCreating(false)} className="p-1.5 bg-red-600/10 text-red-600 hover:bg-red-600 hover:text-white rounded-lg" title={t("admin.banners.categories.cancel_btn")}><X size={16} /></button>
                      </div>
                    )}
                    ...
                    {filteredCategories.map(cat => (
                      <div key={cat.id} className="space-y-2">
                        {/* Parent Item */}
                        <div className={`p-3 border transition-all flex items-center justify-between gap-3 rounded-2xl ${editingId === cat.id ? "border-emerald-500 bg-emerald-500/5 shadow-xl shadow-emerald-500/5 font-bold" : "border-border hover:border-emerald-500/50 group bg-muted/20"}`}>
                          ...
                          <div className="flex items-center gap-1 shrink-0">
                            {editingId === cat.id ? (
                              <>
                                <button onClick={() => handleSave(cat.id)} className="p-1.5 text-emerald-600 hover:bg-emerald-600/10 rounded-lg" title={t("admin.banners.categories.save_btn")}><Save size={16} /></button>
                                <button onClick={() => setEditingId(null)} className="p-1.5 text-red-500 hover:bg-red-500/10 rounded-lg" title={t("admin.banners.categories.cancel_btn")}><X size={16} /></button>
                              </>
                            ) : deletingId === cat.id ? (
                              <div className="flex items-center gap-1 animate-in zoom-in-50">
                                  <button
                                    onClick={() => handleDelete(cat.id)}
                                    className="h-8 px-3 bg-red-600 text-white font-black uppercase text-[10px] tracking-widest hover:bg-red-700 rounded-lg"
                                  >
                                    {t("admin.banners.categories.delete_confirm")}
                                  </button>
                                  <button
                                    onClick={() => setDeletingId(null)}
                                    className="h-8 px-3 bg-muted text-foreground font-black uppercase text-[10px] tracking-widest hover:bg-card border border-border rounded-lg"
                                  >
                              ...
                              <>
                                  <button onClick={() => startCreate(cat.id)} className="p-1.5 text-emerald-600 hover:bg-emerald-600/10 rounded-lg sm:opacity-0 group-hover:opacity-100 transition-opacity" title={t("admin.banners.categories.add_child_hint")}><Plus size={16} /></button>
                                  <button onClick={() => startEdit(cat)} className="p-1.5 text-blue-600 hover:bg-blue-600/10 rounded-lg sm:opacity-0 group-hover:opacity-100 transition-opacity" title={t("admin.banners.categories.edit_hint")}><Edit2 size={16} /></button>
                                  <button onClick={() => setDeletingId(cat.id)} className="p-1.5 text-red-600 hover:bg-red-600/10 rounded-lg sm:opacity-0 group-hover:opacity-100 transition-opacity" title={t("admin.banners.categories.delete_hint_btn")}><Trash2 size={16} /></button>
                              </>
                            )}
                          </div>
                        </div>
