...
export default function AdminProductsPage() {
  ...
  return (
    <div className="p-6 md:p-10 space-y-10 bg-background max-w-7xl mx-auto overflow-hidden">
      ...
      {/* Header */}
      <div className="flex flex-col items-center justify-center text-center border-b border-border pb-6 md:pb-8 gap-4">
        ...
        <div className="bg-muted px-6 md:px-8 py-3 md:py-4 border border-border flex flex-col items-center rounded-2xl">
      ...
      {/* Toolbar */}
      <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-center bg-card border border-border p-4 rounded-2xl shadow-sm">
        <div className="md:col-span-5 lg:col-span-6 relative">
          ...
          <input
            ...
            className="w-full pl-12 pr-4 py-3 bg-muted/30 border border-transparent focus:border-primary focus:bg-background rounded-xl outline-none font-bold text-sm transition-all"
          />
        </div>
        <div className="md:col-span-3 lg:col-span-3">
          <button
            onClick={() => setShowCategoryManager(true)}
            className="w-full flex items-center justify-center gap-2 py-3 bg-primary text-white font-black uppercase text-[10px] tracking-widest hover:opacity-90 active:scale-95 transition-all rounded-xl shadow-md active:shadow-none"
          >
            <Package size={18} />
        ...
        <div className="md:col-span-4 lg:col-span-3 flex items-center gap-2 text-wrap w-full">
          ...
          <select
            ...
            className="flex-1 w-full bg-muted/30 border border-transparent focus:border-primary px-4 py-3 rounded-xl outline-none font-black uppercase text-xs tracking-widest cursor-pointer transition-all truncate"
          >
        ...
        <div className="overflow-x-auto scrollbar-hide">
          <table className="w-full text-left text-sm border-separate border-spacing-0">
            ...
            <tbody className="divide-y divide-border">
              {loading ? (
                <tr><td colSpan={4} className="px-6 py-20 text-center"><div className="w-8 h-8 bg-muted animate-pulse rounded-full mx-auto" /><p className="text-[10px] font-bold uppercase opacity-40 mt-2">{t("loading")}</p></td></tr>
              ) : products.length === 0 ? (
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} className="relative w-full max-w-5xl bg-card border border-border rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[95vh]">
                ...
                <div className="p-4 md:p-10">
                  <div className="grid grid-cols-1 lg:grid-cols-[350px_1fr] gap-6 md:gap-10 items-start">
                    ...
                    <div className="space-y-4">
                      <div className="aspect-square bg-white border border-primary/10 rounded-2xl flex items-center justify-center p-8 relative overflow-hidden group">
                      ...
                      <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
                        {selectedProduct.media?.map((m: any, i: number) => (
                          <div key={i} className="w-16 h-16 md:w-20 md:h-20 border border-border rounded-xl overflow-hidden shrink-0 opacity-70 hover:opacity-100 transition-all cursor-pointer">
                    ...
                    <section className="space-y-6">
                      ...
                      {loadingReviews ? (
                        <div className="p-10 text-center">
                          <div className="w-8 h-8 bg-muted animate-pulse rounded-full mx-auto" />
                        </div>
                      ) : reviews.length === 0 ? (
                        <div className="p-10 border border-dashed border-border text-center opacity-30 rounded-3xl">
              ...
              <div className="p-4 md:p-8 border-t border-border bg-muted/30 flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4 md:gap-6">
                <div className="flex flex-col sm:flex-row gap-3 md:gap-4">
                  ...
                  {!selectedProduct.deleted_at ? (
                    <button
                      ...
                      className="px-8 py-3.5 border border-red-500 text-red-500 hover:bg-red-500 hover:text-white rounded-2xl font-black uppercase text-[11px] tracking-widest transition-all flex items-center justify-center gap-2"
                    >
                      <Trash2 size={18} />
                      {t("admin.delete")}
                    </button>
                    ...
                    <button
                      ...
                      className="px-8 py-3.5 border border-emerald-500 text-emerald-500 hover:bg-emerald-500 hover:text-white rounded-2xl font-black uppercase text-[11px] tracking-widest transition-all flex items-center justify-center gap-2"
                    >
                ...
                <button
                  ...
                  className="px-10 py-3.5 bg-muted text-muted-foreground hover:bg-card border border-border rounded-2xl font-black uppercase text-[11px] tracking-widest transition-all text-center"
                >
      ...
      <AnimatePresence>
        {showSellerModal && selectedSeller && (
          <div className="fixed inset-0 z-[120] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} className="relative w-full max-w-4xl bg-card border border-border rounded-[2rem] shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">

              {/* Modal Header */}
              <div className="p-4 md:p-6 border-b border-border bg-muted/30 flex flex-col sm:flex-row items-center justify-between gap-4">
                <div className="flex flex-col sm:flex-row items-center gap-4 text-center sm:text-left text-wrap">
                  <div className="w-16 h-16 border border-border bg-background rounded-2xl flex items-center justify-center font-black text-2xl text-primary shadow-sm shrink-0">
                    {selectedSeller.name?.charAt(0).toUpperCase()}
                  </div>
                  <div className="min-w-0">
                    <h2 className="text-2xl md:text-3xl font-black uppercase tracking-tight truncate">{selectedSeller.name}</h2>
                    <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2 mt-1">
                      <span className={`px-3 py-0.5 border border-purple-500/20 rounded-full font-black text-[9px] tracking-widest uppercase bg-purple-500/10 text-purple-500`}>
                        {t("roles.seller").toUpperCase()}
                      </span>
                      <span className="text-[10px] font-bold text-muted-foreground uppercase opacity-40">UID: {selectedSeller.id}</span>
                    </div>
                  </div>
                </div>
                <button onClick={() => setShowSellerModal(false)} className="p-2 md:p-3 hover:bg-muted rounded-full transition-all">
                  <X size={24} />
                </button>
              </div>
                      ...
                      <div>
                        ...
                        <div className="space-y-4 text-wrap">
                          <div className="flex items-center gap-4 group">
                            <div className="w-10 h-10 border border-border bg-muted rounded-xl flex items-center justify-center group-hover:bg-primary/10 transition-colors shrink-0">
                              <Mail size={18} />
                            </div>
                            <div className="min-w-0 flex-1">
                    ...
                    <div>
                      ...
                      <div className="space-y-3">
                        {selectedSeller.addresses?.length > 0 ? selectedSeller.addresses.map((addr: any) => (
                          <div key={addr.id} className="p-4 border border-border bg-muted/20 rounded-2xl relative group overflow-hidden text-wrap">
                            <div className="absolute top-0 right-0 px-3 py-1 bg-border text-[8px] font-black uppercase text-white tracking-widest rounded-bl-xl">{addr.type}</div>
                            ...
                            {addr.is_default && (
                              <div className="mt-2 inline-block px-2 py-0.5 border border-primary text-[8px] font-black uppercase text-primary tracking-tighter rounded-full">{t("profile_page.default")}</div>
                            )}
                          </div>
                        )) : (
                          <div className="p-10 border border-dashed border-border text-center opacity-40 rounded-3xl">
              ...
              {!loadingSeller && (
                <div className="p-4 md:p-8 border-t border-border bg-muted/30 flex flex-col md:flex-row gap-4 items-stretch md:items-center justify-between">
                  <div className="flex flex-col sm:flex-row gap-3 md:gap-4">
                    <Link
                      href={`/admin/inbox?userId=${selectedSeller.id}`}
                      className="flex items-center justify-center gap-3 bg-blue-600 text-white px-8 py-3.5 rounded-2xl font-black uppercase text-xs tracking-widest shadow-xl shadow-blue-500/20 hover:scale-105 active:scale-95 transition-all"
                    >
                    ...
                    <button
                      ...
                      className={`flex items-center justify-center gap-3 ${selectedSeller.status === 'banned' ? 'bg-emerald-600 shadow-emerald-500/20' : 'bg-slate-900 shadow-slate-900/20'} text-white px-8 py-3.5 rounded-2xl font-black uppercase text-xs tracking-widest shadow-xl hover:scale-105 active:scale-95 transition-all`}
                    >
                  ...
                  <button
                    ...
                    className="flex items-center justify-center gap-3 border border-red-500 text-red-500 px-8 py-3.5 rounded-2xl font-black uppercase text-xs tracking-widest hover:bg-red-500 hover:text-white transition-all shadow-xl shadow-red-500/10"
                  >
                    <Trash2 size={18} />
      ...
      <AnimatePresence>
        {showBanModal && (
          <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }} className="relative w-full max-w-md bg-card border border-red-500/20 rounded-[2.5rem] shadow-2xl p-6 md:p-10 space-y-8 overflow-hidden">
              <div className="flex flex-col items-center text-center space-y-4">
                <div className="w-20 h-20 bg-red-500/10 rounded-full flex items-center justify-center text-red-500 shadow-inner">
                  <ShieldAlert size={40} />
                </div>
                <h2 className="text-2xl font-black uppercase tracking-tight text-red-600">{t("admin_products.ban_product")}</h2>
                <p className="text-xs font-bold text-muted-foreground uppercase leading-relaxed max-w-[280px] mx-auto">{t("admin_products.confirm_ban")}</p>
              </div>

              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-2">{t("admin_products.ban_reason_placeholder")}</label>
                <textarea
                  value={banReason}
                  onChange={(e) => setBanReason(e.target.value)}
                  className="w-full bg-muted/30 border border-border focus:border-red-500 rounded-2xl p-5 outline-none font-medium text-sm transition-all min-h-[120px] resize-none"
                  placeholder="..."
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <button
                  ...
                  className="py-4 bg-muted hover:bg-card border border-border rounded-2xl font-black uppercase text-[11px] tracking-widest transition-all"
                >
                  {t("profile_page.cancel")}
                </button>
                <button
                  ...
                  className="py-4 bg-red-600 text-white rounded-2xl font-black uppercase text-[11px] tracking-widest shadow-xl shadow-red-500/20 hover:scale-105 active:scale-95 disabled:opacity-50 disabled:scale-100 transition-all flex items-center justify-center gap-2"
                >
                  {isBanning ? <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <Ban size={16} />}
